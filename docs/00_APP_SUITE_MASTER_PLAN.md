...
App không phải chỉ là công cụ image-to-PDF. Feature image optimizer là feature đầu tiên vì đã có workflow thực tế, script thử nghiệm, output sếp hài lòng và bộ test `Sao ke GD`.

Quyết định boundary mới phải đọc cùng tài liệu này:
...
|---|---|---|---|
| 01 | Image Optimizer | Làm trước | Nén/tối ưu ảnh, ví dụ AVIF |
| 02 | File Merge / PDF Builder | Sau Feature 01 | Gộp ảnh/file/PDF thành PDF theo thứ tự |
| 03 | PDF Compressor | Sau Feature 01 | Preview PDF, chỉnh setting nén, nén lại đến khi final |
| 04 | PDF Converter | Sau Feature 01 | PDF sang ảnh, ảnh sang PDF, format khác |
| 05 | PDF Split | Sau Feature 01 | Tách PDF |
| 06 | Word Tools | Sau MVP | Combine/convert Word |
| 07 | Excel Tools | Sau MVP | Combine/convert Excel |
| 08 | Batch Workspace | Sau MVP | Xử lý nhiều folder/job |
...
-> Review dung lượng
-> Có thể nén lại ảnh
-> Có thể gửi batch sang File Merge / PDF Builder
-> Có thể chạy automation Gộp và nén để đi tiếp sang PDF Compressor
```
...
- Đã kiểm chứng thực tế: 10 ảnh JPG gốc khoảng 8.32 MiB, 10 ảnh AVIF khoảng 1.36 MiB.
- Đã xác minh rule quan trọng cho các feature sau: PDF RGB q12/q13/q15 nằm trong vùng khoảng 1.2-1.5 MiB, không Gray mặc định, không ép A4, chỉnh q để đạt dung lượng.
- Có giá trị dùng ngay.

## 5. Core app không được phụ thuộc vào một feature

Core app phải là shell chung, không hardcode riêng cho Image Optimizer, File Merge hay PDF Compressor.
...
- Global log/status.
- Feature 01 Image Optimizer hoàn chỉnh.
- File Merge / PDF Builder và PDF Compressor có thể ở trạng thái placeholder nếu chưa implement, nhưng boundary phải rõ.
...
4. Implement Feature 01 Image Optimizer.
5. QA Feature 01 bằng bộ `Sao ke GD`.
6. Sau khi Feature 01 ổn, mở Feature 02: File Merge / PDF Builder.
7. Sau đó mở Feature 03: PDF Compressor.
