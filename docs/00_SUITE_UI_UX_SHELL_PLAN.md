...
|---|---|---|
| ImagePdfOptimizer | Ảnh -> PDF nhẹ | Active, làm trước |
| PdfCompressor | Nén PDF | Placeholder |
| PdfConverter | Convert PDF | Placeholder |
| PdfCombineSplit | Gộp/Tách PDF | Placeholder |
...
Giữ style thống nhất:

- Primary đỏ đậm `#8B1E2D`.
- Nền chính trắng.
- Sidebar/panel xám nhạt.
- Warning amber.
- Error red orange.
- Success green.
- Font `Segoe UI`.

Không mỗi feature tự chọn palette riêng.
...
- Đặt tên solution/project theo riêng `ImagePdfOptimizer` nếu đang build app suite.
