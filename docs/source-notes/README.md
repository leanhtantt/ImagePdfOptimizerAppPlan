...
Bộ file `00-05` là bản đã gom, tách và chuẩn hoá cho **Feature 01: Image Optimizer**.

File `09_FEATURE_BOUNDARY_AND_AUTOMATION_DECISION.md` là quyết định mới nhất về ranh giới feature:
...
Nếu các file plan cũ còn câu kiểu `Image -> AVIF -> PDF Optimizer`, `AVIF -> PDF RGB -> chỉnh q -> final`, hoặc `Combine PDF nằm trong Feature 01`, phải hiểu là đã bị thay thế bởi file `09`.
...
- Gộp file thuộc `File Merge / PDF Builder`.
- Nén PDF thuộc `PDF Compressor`.
- Image Optimizer chỉ expose handoff/automation `Gộp file` và `Gộp và nén`.
...
File `09_FEATURE_BOUNDARY_AND_AUTOMATION_DECISION.md` là checklist boundary bắt buộc trước khi giao agent khác code workflow liên quan Image Optimizer, File Merge hoặc PDF Compressor.
